...
| Topology | 同一 Fab 內固定的 Phase（Node）集合，≤10 Node，任一 Node 可為自身資料的 Source；第一版不支援跨 Fab 同步，也不支援運行期間新增、移除或變更同步對象 |
...
- Node independence 不代表 shared infrastructure 故障時所有 Node 都可保持正常。已確認的跨 Node 共用依賴僅有 Control Plane 與 AD / DNS；各 Phase 擁有獨立 NAS，Storage 不是 Fab 內的 correlated failure domain。設計若新增任何共用依賴，必須補列其故障影響範圍。
...
| Term | Definition |
| --- | --- |
| Fab | 一個廠區，包含多個 Node（Phase）；只是 Node 的分組，不是同步單位 |
| Node | 可獨立運行的部署單位，對應一個 Fab 內的一個 Phase；擁有自己的 Local Storage 與 state |
...
| Workload envelope | 每 Fab ≤10 Node；每 Node 每日 ≥10⁵ 檔案；檔案 MB 級；推算單一 Target 24 小時累積 10²–10³ GB。持續／尖峰 files/sec、bytes/sec、file size distribution、最大檔案大小：TBD |
...
- A / B / C 為同一 Fab 內的三個 Phase（Node），各自獨立 NAS，健康，初始固定 Policy 已設定。
