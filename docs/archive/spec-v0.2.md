...
Version: 0.2  
Status: Design baseline；量化 SLO 與容量參數須於正式驗收前完成  
Date: 2026-09-21
...
| Topology | 初始設定固定 Node 與同步對象；第一版不支援運行期間新增、移除或變更同步對象 |
...
- Node independence 不代表 shared infrastructure 故障時所有 Node 都可保持正常；設計必須列出實際共用依賴與故障影響範圍。
...
| Term | Definition |
| --- | --- |
| Node | 可獨立運行的部署單位，可對應 Site / Data Center / Fab |
| Local Storage | 本 Node 使用的 NAS / NFS Storage；不代表應用主機本機磁碟 |
| Source Node | 某份原始資料的權威來源 Node |
| Target Node | 初始設定中需要取得該資料副本的 Node |
| File identity | 能唯一識別來源及不可變檔案版本的識別；不能僅假設跨 Node 路徑唯一 |
| Writing | 尚未完成、不可被正式消費或複製的寫入狀態 |
| Source Ready | 完成內容寫入、完整性基準、持久化與可恢復接受條件的狀態 |
| Target Ready | 完成傳輸、完整性驗證、持久化及正式發布的狀態 |
| Required targets | 依初始固定 policy，某份資料必須抵達的 Target 集合 |
| Replication obligation | 一份 File identity 對一個 Required target 的同步義務 |
| Replication Lag | Source Ready 至對應 Target Ready 的經過時間，包含等待、重試及停機 |
| Backlog | 所有尚未完成的同步義務，包含 blocked、quarantined 及恢復中的工作 |
| Reconciliation | 對照應有資料集合與實際狀態，發現缺漏、不一致及未建立的同步工作 |
| Control Plane | 管理初始 topology、允許變更的 operational policy、設定版本與整體可視性 |
| Data Plane | 執行檔案同步、驗證、重試、reconciliation 與恢復的運行路徑 |
...
Contract 須明確定義 write、完成寫入／Ready、read、結果查詢、可重試錯誤與結果待確認的語意；不規定 API signature。
...
已知應抵達但尚未 Target Ready 的資料須回報 DATA_NOT_READY 或等效狀態。無法查證存在性時須回報 UNAVAILABLE／UNKNOWN 或等效狀態，不得誤報為已確認不存在。
...
每份 File identity 具有唯一 Source authority；Ready 後內容不再修改。未來若產生不同內容，必須視為新的 File identity，不覆寫既有 Ready 內容。
...
對持續新增資料，驗收須使用明確截止點形成應有集合。檢查週期、完整掃描期限及內容驗證覆蓋須於正式驗收前定義；不得宣稱涵蓋未實際檢查的資料。
...
資料完成後的業務保存期限由資料擁有者另訂；Framework 的完成證據與檢查範圍須與該期限一致。跨 Node 刪除不在第一版範圍。
...
Oldest unfinished age 與 Replication Lag 為核心指標。若沿用 oldest_pending_age 名稱，仍須涵蓋所有未完成狀態。
...
| Workload envelope | 持續／尖峰 files/sec、bytes/sec、file size distribution、最大檔案大小、Node / Target 數：TBD |
...
| Reconciliation | 完整檢查範圍、週期、最長偵測時間、可修復差異恢復期限：TBD |
| Capacity | 保留容量、暫存空間、state 容量、告警及拒絕新寫入門檻：TBD |
| Unknown outcome | 必要服務恢復後的查證期限：TBD |
| Measurement | 時間來源、允許時鐘誤差、統計窗口與樣本規則：TBD |
...
- A / B / C 健康，初始固定 Required targets 已設定。
