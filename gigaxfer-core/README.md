...
2. `finalizeWrite()` 回 `Success` 才 commit 業務交易；`PendingConfirmation` 重呼同一 handle 的 `finalizeWrite()` 直到確定；`Failure` 視為交易失敗，且是終態：同一 handle 之後每次 `finalizeWrite()` 都原樣回第一次的 `Failure`，重試不會改變已回報的結論。例外是本 handle 送出的 link-key 逾時、結果未知時：之後查證（stat-key、read-manifest、digest-key）遇到 I/O 錯誤回 `PendingConfirmation`，不回 `Failure`，故障解除後重呼可收斂為 `Success`。未知狀態只延續到先前的 link 全部結束、且 stat-key 確認正式檔不存在為止；之後的 I/O 錯誤是確定的 `Failure`（終態），新 link 再逾時才重新成為未知。第一次呼叫 `finalizeWrite()` 時內容立即定案：之後 `stream()` 的任何寫入都在外層入口丟 `IOException`，連小寫入也不會先進 buffer。這包含 fsync 尚未回覆、close 失敗及已回 `Success` 的狀態；被拒寫不會把 handle 毒化，重呼仍沿原 Finalize 序列收斂。
3. **Handle 中毒（poisoned）**：`stream().write(...)` 只要失敗，handle 就從此中毒——池滿／timeout 丟 `NfsUnavailableException`（`IOException` 子類），其餘寫入失敗（ENOSPC、EDQUOT、EIO、ESTALE…）原樣丟出該 `IOException`。`finalizeWrite()` 內部第①步的 `stream.flush()` 也屬於寫入：flush 失敗或池滿／timeout 同樣毒化 handle，**該次** `finalizeWrite()` 就直接回 `Failure(IO, "stream failed at " + <op>)`（不先回 `PendingConfirmation`），之後每次呼叫也一樣，不再嘗試發布、不宣告；暫存以 best-effort 清除（會再發 unlink op）。Application 必須把這視為交易失敗、拋棄此 handle、用新的 `beginWrite()` 重來——這是安全的，因為 commit point（③ link-key）從未被踩到。只有 flush 之後的純 NFS 操作（fsync 起：fsync、manifest、link 等，不經 `stream()`）池滿或 timeout 才回 `PendingConfirmation`，handle 不中毒，重呼 `finalizeWrite()` 會照原序列重試（`FinalizeRetryTest`、`FinalizeUnderPressureTest` 第三個案例）。
4. 交易重跑直接 `beginWrite` + `finalizeWrite`，冪等保證不重複；不需先查。
5. `discard()` 只允許在第一次 `finalizeWrite()` 呼叫前；Finalize 已開始後（包含 `PendingConfirmation`）丟 `IllegalStateException`，此時只能繼續重呼 `finalizeWrite()`，不能放棄；永久放棄的 `PendingConfirmation` handle 其暫存由清道夫依 TTL 清（D35）。write 失敗而中毒、尚未開始 Finalize 的 handle，或 Finalize 已回 `Failure` 的 handle（暫存已由 library 清除）仍可呼叫 discard。`discard()` 的刪除只要失敗（池滿／timeout 丟 `NfsUnavailableException`，或 EACCES、EIO、ESTALE 等一般 `IOException`）都**不算已放棄**：暫存可能仍在，handle 轉為中毒（`finalizeWrite()` 只回 `Failure`、不發布），重呼 `discard()` 會再刪一次（`finalizeWrite()` 不會代為清暫存，只回 `Failure(IO, "discard delete unresolved at …")`）；刪除真正完成才進入已放棄狀態，之後重呼為 no-op。已回 `Failure` 的 handle 在 `discard()` 之後重呼 `finalizeWrite()` 仍回原本那個 `Failure`。
...
| F1 | crash 場景（①②步之間 process 死亡），in-process 測不到，歸 P13／P14；重跑交易走冪等路徑 |
| F18 | core 半邊已涵蓋（`BoundedNfsExecutorTest`、`FinalizeUnderPressureTest`：無等待、卡住占槽不釋放、滿即拒、回 PENDING／UNAVAILABLE）；真實 NAS hang 與 systemd／health 待 P10／P13 |
